...
/**
   \brief Display the internal representation of the given logical context in the standard error output. 
...
/**
   \brief Similar to #yices_assert_weighted, but the weight is considered to be infinite.
*/
...
/**
  ...
  and compute its cost. If \c random is non-zero, then random search is used,
...
/**
   \brief Evaluate a formula in a model.  Run #yices_check to generate a model.
   ...
   \c l_undef means the model does not have enough information -
        typically this is due to a function application
	e.g., the model defines (f 1) and (f 2), but the formula
	references (f 3)
*/
...
/**
   \brief Compute the maximal satisfying assignment for the asserted weighted constraints.
...
/**
   ...
   \return l_false if such model doesn't exist.
 */
...
/**
   \brief Return the assignment for the variable \c v. The result is \c l_undef if the value
   of \c v is a "don't care".
...
/**
   ...
   because the numerator or denominator are too big
...
/**
   ...
   point number. The value is stored in <tt>* value<tt>. Returns 1 on success.
...
/**
   \brief Return 1 (0) if the given assertion id was satisfied (not satisfied) in the given module. 
...
/**
   \brief Return the cost of the given model. 
...
/**
   \brief Return a new boolean variable declaration. It is an error to
...
/**
   \brief Create an iterator that can be used to traverse the variables (var_decl objects) in the 
   given logical context. An iterator is particulary useful when we want to extract the assignment
...
/**
   \brief Return a variable declaration associated with the given name. Return 0 if there is no
   variable declaration associated with the given name.
*/
...
/**
   ...
   The type of \c f must be a function-type, and its arity should
...
/**
  ...
  then the result is a bitvector of \c concat size \c n1 + \c n2.  Bit
