...
CPdf::real_type CSingular::func_def( const real_type& x ) const {
  if ( x == m_lower )
    return( 1 );
  return( 0 );
}

CPdf::real_type CSingular::rand_def() {
  return( m_lower );
}
...
/////////////////////////////////////////////////////////////////////////////
CPdf::real_type CSingular::expected_value_def() const {
  return( m_lower );
}
...
CSingular::CSingular( const real_type& l,
		      const real_type& upsilon )
  : CPdf( l, l, upsilon ) {
  // DO NOTHING
}
  
CSingular::CSingular( const CSingular& c ) 
: CPdf( c ) {
  // Do Nothing
}
...
void CSingular::serializeOut( ostream& s ) const {
  s << "<" << m_lower << "," << m_upper << "> => ";
  s << "S(" << m_lower << ") : ";
  s << m_norm;
}
