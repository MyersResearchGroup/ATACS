...
##############################################################################
# Convert an integer to binary for a specified number of digits.
##############################################################################
def dec2bin(num,bits):
	return "".join([str((num >> y) & 1) for y in range(bits)])
...
def addMetaBins(g,varsL,divisionsL):
	...
	metaPlaceL = []
	for p in placeL:
		if p.isRateP() and p.isEdgeP(divisionsL):
			print "Edge place:"+str(p)
			#addEdgePlace(g,p,varsL,divisionsL,metaPlaceL)
	addJumpTransitions(metaPlaceL,varsL,g,divisionsL)
	#connectMetaBins(metaPlaceL,varsL,g)
	print "addMetaBins:end"

##############################################################################
# Add places and transitions to support the meta-bins for limited variables.
##############################################################################
def addEdgePlace(g,p,varsL,divisionsL,metaPlaceL):
	for i in range(1,2**(len(varsL)-1)):
		decVal = dec2bin(i,len(varsL)-1)
		addBin = True
		#if all of the 1's in the binary value contain bits of the bin encoding that on the edge then generate a place for that combination
		for j in range(len(decVal)):
			print "decVal:" + decVal + " j:" + str(j) + " decVal[j]:" + decVal[j] + " p.binEncodingL[j]:" + str(p.binEncodingL[j]) + " len(divisionsL[j+1]):" + str(len(divisionsL[j+1]))
			if decVal[j] == '1':
				if p.binEncodingL[j] != 0:
					if p.binEncodingL[j] != len(divisionsL[j+1]):
						addBin = False
						print "addBin = False (1)"
						break
					elif p.binEncodingL[j] == len(divisionsL[j+1]):
						if not maxVarValL[j+1]:
							addBin = False
							print "addBin = False (2)"
							break
				elif p.binEncodingL[j] == 0:
					if not minVarValL[j+1]:
						addBin = False
						print "addBin = False (4)"
						break
		if addBin:
			print "Adding a place! -- decVal:" + decVal
			place = Place(varsL,RATE)
			metaPlaceL.append(place)
			place.ratesL = copy.deepcopy(p.ratesL)
			for j in range(len(decVal)):
				print "j:" + str(j) + " decVal[j]:" + decVal[j]
				if decVal[j] == '1':
					if p.binEncodingL[j] == 0 and minVarValL[j+1]:
						print "If1: -1" 
						place.binEncodingL.append(-1)
						place.rmRateLtZero(j+1)
					elif p.binEncodingL[j] == len(divisionsL[j+1]) and maxVarValL[j+1]:
						print "If2: " +  str(len(divisionsL[j+1])+1)
						place.binEncodingL.append(len(divisionsL[j+1])+1)
						place.rmRateGtZero(j+1)
					else:
						print "Else3: " + str(p.binEncodingL[j])
						place.binEncodingL.append(p.binEncodingL[j])
				else:
					print "Else: " + str(p.binEncodingL[j])
					place.binEncodingL.append(p.binEncodingL[j])
			print "key:" + place.keyStr()
			g.placeD[place.keyStr()] = place
			#Add transitions between core bins and meta-bins if appropriate
			print "metabinsOrder:" + str(place.orderMetaBin(divisionsL))
			if place.orderMetaBin(divisionsL) == 1:
				diffL = place.diff(p,varsL)
				addT1 = False
				addT2 = False
				print "diffL:" + str(diffL)
				if place.binEncodingL[diffL[0]-1] == -1:
					if int(place.maxRateInt(diffL[0])) > 0:
						print "Adding T1"
						addT1 = True
					if int(p.minRateInt(diffL[0])) < 0:
						print "Adding T2"
						addT2 = True
				else:
					if int(place.minRateInt(diffL[0])) < 0:
						print "Adding T1"
						addT1 = True
					if int(p.maxRateInt(diffL[0])) > 0:
						print "Adding T2"
						addT2 = True
				if addT1:
					t1 = Transition(len(varsL),p,place)
					g.transitionD[(place.keyStr(),p.keyStr())] = t1
					place.outgoingL.append(t1)
					p.incomingL.append(t1)
				if addT2:
					t2 = Transition(len(varsL),place,p)
					g.transitionD[(p.keyStr(),place.keyStr())] = t2
					p.outgoingL.append(t2)
					place.incomingL.append(t2)
					

##############################################################################
# Determine if a given bin encoding is in the core or on the periphery.
##############################################################################
def isCoreEncoding(binEncodingL,divisionsL):
	for i in range(len(binEncodingL)):
		if binEncodingL == -1 or self.binEncodingL[i] == len(divisionsL[i+1])+1:
			return False
	return True
...
##############################################################################
# Look at each core place and determine if a "jump" transition needs to be 
# added.
##############################################################################
def addJumpTransitions(metaPlaceL,varsL,g,divisionsL):
	placeL = g.placeD.values()
	placeL.sort()
	for p in placeL:
		if p.isRateP():
...
##############################################################################
# Look at all combinations of meta-bins and add transitions between them as 
# appropriate.
##############################################################################
def connectMetaBins(metaPlaceL,varsL,g):
	#Now connect up all of the meta-bins
	print "Connecting up meta-bins"
	print "len:" + str(len(metaPlaceL))
	for pSrc in metaPlaceL:
		for pSync in metaPlaceL:
			print "Testing: p" + str(pSrc.placeNum) + " p" + str(pSync.placeNum)
			diffL = pSrc.diff(pSync,varsL)
			if len(diffL) == 1 and abs(pSrc.binEncodingL[diffL[0]-1] - pSync.binEncodingL[diffL[0]-1]) == 1:
				print "pSrc:" + str(pSrc)
				print "pSync:" + str(pSync)
				if pSrc.binEncodingL[diffL[0]-1] < pSync.binEncodingL[diffL[0]-1]:
					print "Rate:" + pSrc.maxRateInt(diffL[0])
					if int(pSrc.maxRateInt(diffL[0])) > 0:
						t = Transition(len(varsL),pSync,pSrc)
						print "Creating t" + str(t.transitionNum)
						g.transitionD[(pSrc.keyStr(),pSync.keyStr())] = t
						pSrc.outgoingL.append(t)
						pSync.incomingL.append(t)
				else:
					print "Rate:" + pSrc.maxRateInt(diffL[0])
					if int(pSrc.minRate(diffL[0])) < 0:
						t = Transition(len(varsL),pSync,pSrc)
						print "Creating t" + str(t.transitionNum)
						g.transitionD[(pSrc.keyStr(),pSync.keyStr())] = t
						pSrc.outgoingL.append(t)
...
##############################################################################
# Calculate the rate for a trace based .g file.
##############################################################################
def traceGenRates(datL,varsL):
	ratesL = create2Darray(len(datL),len(varsL),None)
	for i in range(len(datL)-1):
		deltaT = datL[i+1][0]-datL[i][0]
		for j in range(1,len(varsL)):
			ratesL[i][j] = (datL[i+1][j]-datL[i][j])/deltaT
	return ratesL

##############################################################################
# Generate the graph for a trace based .g file.
##############################################################################
def generateAccurateTraceGraph(datL,varsL,g):
	prevDMVCvalL = []
	prevP = None
	p = None
	ratesL = traceGenRates(datL,varsL)
	for i in range(len(varsL)):
		prevDMVCvalL.append(None)
	for i in range(len(datL)-1):
		prevP = p
		p = Place(varsL,TRACE)
		g.placeD["p"+str(p.placeNum)] = p
		t = Transition(len(varsL),prevP,p)
		g.transitionD["t"+str(t.transitionNum)] = t
		#Add the transitions and places to the proper lists
		if prevP:
			prevP.outgoingL.append(t)
		p.incomingL.append(t)
		for j in range(1,len(varsL)):
			if varsL[j].dmvc:
				#Advance these with time
				a = AsgnPart()
				a.var = j
				a.valL.append(datL[i+1][j])
				a.delayL.append(datL[i+1][0]-datL[i][0])
				p.asg.append(a)
			else:
				#Advance these with rates
				p.ratesL[j].append(ratesL[i][j])
				p.dmvcVar = j
				p.dmvcTimeL[j].append(datL[i+1][0]-datL[i][0])
	for i in range(1,len(varsL)):
		g.initValL[i].append(datL[0][i])
		if varsL[i].dmvc:
			g.initRateL[i].append(0)
		else:
			g.initRateL[i].append(ratesL[0][i])
	initP = g.placeD.get("p0")
	g.initMarkingL.append(initP)

##############################################################################
# Do normalization for a trace based .g file.
##############################################################################
def normalizeTraceValues(g,varsL):
	normG = copy.deepcopy(g)
	normDivisionsL = copy.deepcopy(divisionsL)
	minDelayVal = 10
	minRateVal = 10
	...
	for p in placeL:
		if p.asg:
			if minDelay != -1:
				if p.a.minDelay() < minDelay:
					minDelay = p.a.minDelay()
				if p.a.maxDelay() > maxDelay:
					maxDelay = p.a.maxDelay()
			else:
				minDelay = p.a.minDelay()
				maxDelay = p.a.maxDelay()
		elif p.dmvcTimeL[p.dmvcVar]:
			if minDelay != -1:
				if a.minDelay() != 0.0 and a.minDelay() < minDelay:
					minDelay = a.minDelay()
				if a.maxDelay() != 0.0 and a.maxDelay() > maxDelay:
					maxDelay = a.maxDelay()
				else:
					minDelay = a.minDelay()
					maxDelay = a.maxDelay()
		#print "p"+str(p.placeNum)+"minDelay:"+str(minDelay)+" maxDelay:"+str(maxDelay)
	#print "minDelay:"+str(minDelay)+" maxDelay:"+str(maxDelay)
	#determine the scaling factor
	rawScaleFactor = abs(minDelayVal/minDelay) #abs() shouldn't be needed here, but we will just be safe
	scaleFactor = 1.0
	for i in range(14):
		...
		scaleFactor *= 10.0
	#print "minDelay is: "+str(minDelay*scaleFactor)+" after scaling by "+str(scaleFactor)
	#check for overflow then multiply all time values by the scaling factor
	if int(maxDelay*scaleFactor) > sys.maxint:
		cStr = cText.cSetFg(cText.YELLOW)
		cStr += "WARNING:"
		cStr += cText.cSetAttr(cText.NONE)
		print cStr+" Delay scaling has caused an overflow."
	...
	for p in placeL:
		...
		if p.isAsgnP():
			for a in p.asg:
				for i in range(len(a.delayL)):
					a.delayL[i] *= scaleFactor
	#divide all rates by the scaling factor
	for p in placeL:
		if p.isRateP():
			for i in range(1,len(varsL)):
				if not varsL[i].dmvc:
					for j in range(len(p.ratesL[i])):
						if p.ratesL[i][j] != "-":
							#print "Scaling rate "+str(p.ratesL[i][j])+" by "+str(scaleFactor)
							p.ratesL[i][j] /= scaleFactor
	for i in range(1,len(varsL)):
		if not varsL[i].dmvc:
			for j in range(len(normG.initRateL[i])):
				normG.initRateL[i][j] /= scaleFactor
	#Determine the scaling factor (using factors of 10)  to bring the smallest rate value above minRateVal
	minRate = -1
	maxRate = -1
	for p in placeL:
		if p.isRateP():
			for i in range(1,len(varsL)):
				if not varsL[i].dmvc:
					if minRate != -1:
						if p.minRate(i) < minRate:
							minRate = p.minRate(i)
						if p.maxRate(i) > maxRate:
							maxRate = p.maxRate(i)
					else:
						minRate = p.minRate(i)
						maxRate = p.maxRate(i)
	#print "minRate is: "+str(minRate)
	#determine the scaling factor
	rawScaleFactor = abs(minRateVal/minRate)
	scaleFactor = 1.0
	for i in range(14):
		if scaleFactor > rawScaleFactor:
			break
	...
	#check for overflow then multiply all rates by the scaling factor
	if int(maxRate*scaleFactor) > sys.maxint:
		cStr = cText.cSetFg(cText.YELLOW)
		cStr += "WARNING:"
		cStr += cText.cSetAttr(cText.NONE)
		print cStr+" Rate scaling has caused an overflow."
	...
	for p in placeL:
		if p.isRateP():
			...
			for a in p.asg:
				for i in range(len(a.valL)):
					#print "p"+str(p.placeNum)+"a.valL["+str(i)+"] *= "+str(scaleFactor)+" = "+str(a.valL[i])
					a.valL[i] *= scaleFactor
	#Scale initial values, initial rates, and thresholds.
	for i in range(1,len(varsL)):
		for j in range(len(normG.initValL[i])):
			#print "Scaling initVal from "+str(normG.initValL[i][j])+" to "+str(normG.initValL[i][j]*scaleFactor)
			normG.initValL[i][j] *= scaleFactor
		if not varsL[i].dmvc:
			for j in range(len(normG.initRateL[i])):
				normG.initRateL[i][j] *= scaleFactor
		for j in range(len(divisionsL[i])):
			normDivisionsL[i][j] *=  scaleFactor
	return normG, normDivisionsL

##############################################################################
# Write out a *.g file for a trace based g file..
##############################################################################
def writeTraceGfile(varsL,g,gFile):
	outputF = open(gFile, 'w')
	outputF.write(".outputs fail\n")
	outputF.write(".dummy")
	transitionL = g.transitionD.values()
	...
	if dmvcVarExists(varsL):
		...
		for p in placeL:
			if p.asg:
				for t in p.outgoingL:
					outputF.write("<t"+str(t.transitionNum)+"=["+p.asg[0].minDelayInt()+","+p.asg[0].maxDelayInt()+"]>")
			else:
				for t in p.outgoingL:
					outputF.write("<t"+str(t.transitionNum)+"=["+p.minTimeInt()+","+p.asg[0].maxTimeInt()+"]>")
		outputF.write("}\n")
	outputF.write("#@.continuous")
	for i in range(1,len(varsL)):
		outputF.write(" "+varsL[i].name)
...
def main():
	...
	parser.add_option("-t", "--trace", action="store_true", dest="trace", help="Enable accurate model generation of an individual simulation trace.  The trace option will generate a new output file for each datFile.")
	...
	if options.trace:
		for i in range(len(datFileL)):
			...
			g = Graph(varsL,"")
			generateAccurateTraceGraph(datL,varsL,g)
			gFile = baseFileL[0] + ".g"
			#Values need to be normalized for the g file
			normG, normDivisionsL = normalizeTraceValues(g,varsL)
			writeTraceGfile(varsL,normG,gFile)
	else:
