...
public:
  ...
  bap_stack_entry( timed_state *ptr_timed_st , 
    ...
    if ( ptr_timed_st == NULL || ptr_Ef == NULL ) {
      char errorStr[] = "ERROR: null stack entry";
  ...
  event_type get_event() {
    if ( ptr_Ef->empty() ) {
      char errorStr[] = "ERROR: empty ptr_Ef in bap_stack_entry::get_event()";
      throw( 
      new error( errorStr ) );
    }
...
timed_state* front( bap_stack& the_stack, 
  ...
  if ( the_stack.empty() ) {
    char errorStr[] = "ERROR: empty stack in front";
    throw( new error( errorStr ) );
  }
...
//////////////////////////////////////////////////////////////////////////
// 
ostream& dump_bap( ostream& s, const tel& t, const rule_set& Rb ) {
...
ostream& dump_bap(  ostream& s, const tel& t, const enabled_set& Een ){
...
void get_rule_set( const event_type& f             ,
  ...
  enabled = is_non_disabling( f, untimed_st, t, 
			      rule_set_inserter( Re ) );
  if( !enabled ){
    char errorStr[] = "ERROR: Firing an event that is not enabled";
    throw( new error( errorStr ) );
  }
}
//
...
rule_set::iterator remove_over_bounded( rule_set& Rb    , 
  ...
  for ( iterator i = first ; i != Rb.end() ; ++i ) {
    ...
    event_type enabled = t.enabled( *i );
    if ( t.is_unbounded( enabling, enabled ) && 
	 Me.is_in_POSET( enabling, EenSize )             ) {
      iter_swap( i, first++ );
      return( first );
    }
  }
...
bool combinatorial_search( enabled_set::iterator current, 
  ...
  if ( current != Een.end() ) {
    rule_iter end = remove_over_bounded( *current, Me, t, Een.size() );
...
void assert_equal( const container& a,
  ...
  if ( a.size() != b.size() ||
    ...
    cout << endl << a_name;
    dump_bap( cout, t, a ) << endl;
    cout << b_name;
    dump_bap( cout, t, b ) << endl << endl;
    char errorStr[] = "ERROR: containers do not match";
    throw( new error( errorStr ) );
  }
}
//////////////////////////////////////////////////////////////////////////
  ...
  for ( event_type i = 1 ; i < (event_type)t.number_events() ; ++i ) {
    ...
    if ( is_TIC_candidate( event, i, t )                       &&
	 ...
	 is_enabled( i, su, t, front_insert_iterator<rule_set>(Re) ) ) {
      ...
      event_set Een;
      // CJM: had to remove this to get to compile on MAC
	...
	ptr_Ef = ptr_Ea;
      ...
      ptr_sc->dump_bap( cout, t ) << endl;
      cout << "Ef: ";
      dump_bap( cout, t, *ptr_Ef ) << endl;
#endif
...
bool is_TDC_candidate( const event_type& ei,
  ...
  // All conditions are satisfied.  This implies that the event 
  // 'candidate' must be handled elsewher for correct TDC implementation.
  return( true );
...
void constrain_TDC_bound( enabled_set::iterator current, 
  ...
  if ( current != Een.end() ) {
    ...
    if ( Mt.is_consistent( Een.size() ) ) {
      ...
      if ( E.size() == 0 ){
	char errorStr[] = "ERROR: constrain_TDC_bound common conflict not found";
	throw( new 
	       error(errorStr));
      }
...
bool is_TDC_issue( const untimed_state& untimed_st, 
		   ...
		   front_insert_iterator<enabled_set> Ec ) {
  bool issue = false;
  for ( event_type i = 1 ; i < (event_type)t.number_events() ; ++i ) {
    rule_set Re;
    if ( i != event                                                    && 
...
void initialize_stack( tel& t, bool verbose, 
...
bool bap_rsg( tel& t                ,
  ...
  try {
    ...
    ptr_tss = new timed_state_space( lg, 1000, t, state_table, opts, bdd_state,
				     use_bdd, verbose );
    if ( ptr_tss == NULL ) {
      char errorStr[] = "ERROR: out of memory";
      throw( new error( errorStr ) );
    }
	...
	resolve_TIC_choice( f, *sc, ptr_tss, t, opts, the_stack );
      ...
      dump_bap( cout, t, su.get_marking() ) << endl;
      cout << endl << "Een: ";
      dump_bap( cout, t, get_Een( the_stack )) << endl;
      cout << endl << "Een_Rb: " << endl;
      dump_bap( cout, t, Een_Rb ) << endl << flush;
#endif
	...
	if ( ptr_tss->add( t, *sc, **i ) ) {
	  ...
	  if ( ptr_Ef->empty() ) {
	    printf("WARNING: Encountered zone with no fireable events.\n");
	    fprintf(lg,"WARNING: Encountered zone with no fireable events.\n");
	    delete ptr_Ef;
	    continue;
	  }
...
#ifdef __PO_COMPARE__
	    cout << "Ea: ";
	    dump_bap( cout, t, *ptr_Ea ) << endl << endl;
#endif
	    delete ptr_Ef;
	    ptr_Ef = ptr_Ea;
	  }
